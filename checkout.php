 ...
 * ─────────────────────────────────────────────────────────────────
 * Integração REAL com:
 *   • M-Pesa Moçambique  → API Vodacom  (developer.mpesa.vm.co.mz)
 *   • e-Mola / Movitel   → API Movitel  (emola.movitel.co.mz)
 *   • Visa / Mastercard  → Gateway simulado (substituir por Stripe/PayU)
 *
 * Fluxo mobile money:
 *   1. Cliente insere número → POST AJAX → ajax_checkout.php
 *   2. ajax_checkout.php chama API → utilizador recebe USSD Push no telemovel
 *   3. Cliente confirma PIN no telemóvel
 *   4. Frontend faz polling a ajax_payment_status.php até confirmação
 *   5. Ao receber INS-0 / sucesso → pedido criado na DB
 * ─────────────────────────────────────────────────────────────────
 */
...
$user_name   = $_SESSION['auth']['nome'] ?? 'Utilizador';
$user_avatar = $_SESSION['auth']['avatar'] ?? null;
$user_phone  = $_SESSION['auth']['telefone'] ?? '';

if (empty($_SESSION['csrf_token'])) {
    $_SESSION['csrf_token'] = bin2hex(random_bytes(32));
}

// ── Modo Buy Now vs Carrinho ──────────────────────────────────────
...
// Normalizar telefone para formato 258XXXXXXXXX
...
:root {
  ...
  --red:#ef4444;--amber:#f59e0b;--blue:#2563eb;--violet:#7c3aed;
  --mpesa:#d62b2b;--emola:#e67e22;
  --sh-sm:0 1px 3px rgba(0,0,0,.1);--sh-md:0 4px 16px rgba(0,0,0,.1);--sh-lg:0 10px 30px rgba(0,0,0,.12);
...
.form-input,.form-select,.form-textarea{
  width:100%;padding:10px 12px;border:1.5px solid var(--bdr);border-radius:var(--r8);
  font-size:14px;color:var(--ink);background:var(--sur);outline:none;
  transition:border-color .15s,box-shadow .15s;
}
.form-input:focus,.form-select:focus,.form-textarea:focus{
  border-color:var(--gr);box-shadow:0 0 0 3px rgba(0,185,107,.12);
}
...
.pay-card{
  padding:14px 10px;border:2px solid var(--bdr);border-radius:var(--r10);
  display:flex;flex-direction:column;align-items:center;justify-content:center;
  gap:7px;text-align:center;transition:all .15s;min-height:88px;
}
...
.pay-name{font-size:11.5px;font-weight:700;color:var(--ink)}
.pay-desc{font-size:10px;color:var(--ink-3)}
.pay-option.mpesa .pay-icon{color:var(--mpesa)}
.pay-option.emola .pay-icon{color:var(--emola)}
.pay-option.visa .pay-icon{color:#1a1f71}
.pay-option.master .pay-icon{color:#eb001b}
.pay-option.manual .pay-icon{color:var(--ink-2)}

/* ════════════════════════════════════════
   PAINEL MOBILE MONEY — M-Pesa / e-Mola
════════════════════════════════════════ */
.mm-panel{
  display:none;margin-top:4px;
  padding:18px;background:var(--bg);border-radius:var(--r12);
...
.mm-panel.mpesa .mm-panel-title i{color:var(--mpesa)}
.mm-panel.emola .mm-panel-title i{color:var(--emola)}

/* Input do número */
.phone-input-wrap{position:relative;display:flex;gap:0}
.phone-prefix{
  flex-shrink:0;
  padding:0 12px;height:44px;
  background:var(--bdr-2);border:1.5px solid var(--bdr);
  border-right:none;border-radius:var(--r8) 0 0 var(--r8);
  display:flex;align-items:center;gap:6px;
  font-size:13.5px;font-weight:700;color:var(--ink-2);
}
.phone-prefix img{width:18px;height:13px;border-radius:2px;object-fit:cover}
.phone-number-inp{
  flex:1;height:44px;padding:0 14px;
  border:1.5px solid var(--bdr);border-radius:0 var(--r8) var(--r8) 0;
  font-size:15px;font-weight:600;color:var(--ink);background:var(--sur);
...
/* Estado do pagamento mobile */
.mm-status{
  display:none;margin-top:16px;
  padding:14px;border-radius:var(--r10);border:1.5px solid;
}
...
.mm-timer{
  ...
  border-radius:50%;border:3px solid var(--bdr);
...
.mm-timer-ring{
  ...
  border-radius:50%;
  background:conic-gradient(var(--gr) calc(var(--p,0)*1%), transparent 0);
...
.card-face{
  ...
  -webkit-backface-visibility:hidden;padding:18px 20px;display:flex;
...
.card-front{background:linear-gradient(135deg,#1a1f71 0%,#2563eb 60%,#1e40af 100%);color:#fff}
.card-front.mc{background:linear-gradient(135deg,#1a1a1a 0%,#2d2d2d 60%,#111 100%)}
...
.mc-circles span{width:26px;height:26px;border-radius:50%}
.mc-circles span:first-child{background:#eb001b}
.mc-circles span:last-child{background:#f79e1b;margin-left:-10px;opacity:.9}
...
.card-input{
  ...
  font-size:14px;color:var(--ink);background:var(--sur);
  outline:none;transition:border-color .15s,box-shadow .15s;
}
...
.card-input.ok{border-color:#22c55e}
...
.manual-panel{
  ...
  background:var(--bg);border-radius:var(--r12);border:1.5px dashed var(--bdr);
  font-size:13px;color:var(--ink-2);line-height:1.7;
  animation:fadeUp .2s var(--ease);
}
.manual-panel.show{display:block}
.ref-box{
  background:var(--sur);border:1.5px solid var(--gr);border-radius:var(--r8);
  padding:10px 14px;margin-top:8px;display:flex;align-items:center;justify-content:space-between;
  font-size:14px;font-weight:800;color:var(--gr);letter-spacing:.5px;
}
...
.po-box{
  ...
  box-shadow:0 30px 80px rgba(0,0,0,.3);min-width:300px;max-width:360px;width:90%;
}
...
.po-title{font-size:16px;font-weight:800;color:var(--ink)}
.po-sub{font-size:12.5px;color:var(--ink-3)}
...
.po-step.active{color:var(--blue);font-weight:700}
.po-step.done{color:#16a34a;font-weight:700}
...
.oi-qty{font-size:11px;color:var(--ink-3)}
.oi-price{font-size:13px;font-weight:700;flex-shrink:0}
...
/* ── Ecrã de sucesso ── */
#successScreen{display:none;position:fixed;inset:0;background:#f3f4f6;z-index:10000;overflow-y:auto}
...
.success-step i{color:var(--gr);flex-shrink:0;width:15px}
.success-step-text{font-size:13px;color:var(--ink-2)}
...
<div class="co-wrap">
  ...
  <div>
    ...
    <div class="co-card">
      ...
      <div class="form-grid" id="deliveryForm">
        ...
        <div class="form-group span2">
          ...
          <input type="text" id="shipAddress" class="form-input" required
        ...
        <div class="form-group">
          ...
          <input type="text" id="shipState" class="form-input"
    ...
    <div class="co-card">
      ...
      <div class="pay-grid">
        <label class="pay-option mpesa">
          <input type="radio" name="pay_method" value="mpesa" checked onchange="selectPay('mpesa')">
          <div class="pay-card">
            <i class="fa-solid fa-mobile-screen pay-icon"></i>
            <div class="pay-name">M-Pesa</div>
            <div class="pay-desc">Vodacom · USSD Push</div>
          </div>
        </label>
        <label class="pay-option emola">
          <input type="radio" name="pay_method" value="emola" onchange="selectPay('emola')">
          <div class="pay-card">
            <i class="fa-solid fa-mobile-screen pay-icon"></i>
            <div class="pay-name">e-Mola</div>
            <div class="pay-desc">Movitel · USSD Push</div>
          </div>
        </label>
        <label class="pay-option visa">
          <input type="radio" name="pay_method" value="visa" onchange="selectPay('visa')">
          <div class="pay-card">
            <i class="fa-brands fa-cc-visa pay-icon"></i>
            <div class="pay-name">Visa</div>
            <div class="pay-desc">Cartão crédito</div>
          </div>
        </label>
        <label class="pay-option master">
          <input type="radio" name="pay_method" value="mastercard" onchange="selectPay('mastercard')">
          <div class="pay-card">
        ...
        <label class="pay-option manual">
          <input type="radio" name="pay_method" value="manual" onchange="selectPay('manual')">
          <div class="pay-card">
            <i class="fa-solid fa-building-columns pay-icon"></i>
            <div class="pay-name">Transferência</div>
            <div class="pay-desc">Bancária / Manual</div>
          </div>
        </label>
      </div>
      ...
      <div id="panel-mpesa" class="mm-panel mpesa show">
        ...
        <div>
          <div class="phone-input-wrap">
            ...
            <input type="tel" id="mpesa-phone" class="phone-number-inp"
              placeholder="84 000 0000" maxlength="12" inputmode="numeric"
              value="<?= esc(substr($prefill_phone_norm,3)) ?>"
              oninput="formatPhone(this)">
          </div>
          <div class="phone-hint">
            <i class="fa-solid fa-circle-info"></i>
            Receberá um USSD Push no seu telemóvel para confirmar com o PIN M-Pesa
          </div>
        </div>
      ...
      <div id="panel-emola" class="mm-panel emola">
        ...
        <div>
          <div class="phone-input-wrap">
            ...
            <input type="tel" id="emola-phone" class="phone-number-inp"
              placeholder="86 000 0000" maxlength="12" inputmode="numeric"
              value="<?= esc(substr($prefill_phone_norm,3)) ?>"
              oninput="formatPhone(this)">
          </div>
          <div class="phone-hint">
            <i class="fa-solid fa-circle-info"></i>
            Receberá um USSD Push no seu telemóvel para confirmar com o PIN e-Mola
          </div>
        </div>
        <div class="mm-status" id="emola-status"></div>
      </div>

      <!-- ═══ PAINEL CARTÃO (Visa/MC) ═══════════════════════════ -->
      <div id="panel-card" class="card-panel">
          ...
          <div class="card-3d" id="card3d">
            <div class="card-face card-front" id="cardFace">
              <div class="card-top">
                ...
                <div class="card-brand-logo" id="cardBrandLogo">
                  <i class="fa-solid fa-credit-card" style="font-size:20px;opacity:.5"></i>
                </div>
              </div>
              <div class="card-num-disp" id="cardNumDisp">•••• •••• •••• ••••</div>
              <div class="card-bottom">
                <div>
                  <div class="card-lbl">Titular</div>
                  <div class="card-holder-name" id="cardNameDisp">NOME NO CARTÃO</div>
                </div>
                <div>
                  <div class="card-lbl" style="text-align:right">Validade</div>
                  <div class="card-exp-val" id="cardExpDisp">MM/AA</div>
                </div>
              </div>
            </div>
            <div class="card-face card-back">
              ...
              <div style="padding:0;position:relative;z-index:1">
                <div class="card-lbl" style="opacity:.5;letter-spacing:.7px;font-size:9px;margin-bottom:4px">CVV</div>
                <div class="card-cvv-strip" id="cardCvvDisp">•••</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="card-form">
          <div class="card-input-wrap">
            <input type="text" id="cardNumber" class="card-input num" placeholder="1234 5678 9012 3456"
              maxlength="23" autocomplete="cc-number" inputmode="numeric"
              oninput="onCardNum(this)" onfocus="flipToFront()">
            <i class="fa-solid fa-credit-card ci-icon"></i>
            <span class="ci-brand" id="cardBrandBadge"></span>
          </div>
          <div class="card-input-wrap">
            <input type="text" id="cardName" class="card-input" placeholder="Nome como aparece no cartão"
              maxlength="26" autocomplete="cc-name"
              oninput="onCardName(this)" onfocus="flipToFront()">
            <i class="fa-solid fa-user ci-icon"></i>
          </div>
          <div class="card-row">
            <div class="card-input-wrap">
              <input type="text" id="cardExpiry" class="card-input" placeholder="MM/AA"
                maxlength="5" autocomplete="cc-exp" inputmode="numeric"
                oninput="onCardExp(this)" onfocus="flipToFront()">
              <i class="fa-regular fa-calendar ci-icon"></i>
            </div>
            <div class="card-input-wrap">
              <input type="text" id="cardCvv" class="card-input" placeholder="CVV"
                maxlength="4" autocomplete="cc-csc" inputmode="numeric"
                oninput="onCardCvv(this)" onfocus="flipToBack()" onblur="flipToFront()">
              <i class="fa-solid fa-lock ci-icon"></i>
            </div>
          </div>
          <div class="card-secure">
            <i class="fa-solid fa-shield-halved"></i>
            Dados encriptados com SSL 256-bit. Nunca armazenados nos nossos servidores.
          </div>
        </div>
      </div>
      ...
      <div id="panel-manual" class="manual-panel">
        ...
        <div class="ref-box" style="font-size:13px">
          Use o número do pedido como referência
          <span style="font-size:11px;color:var(--ink-3)">(gerado após confirmar)</span>
        </div>
        Envie o comprovativo para <strong>pagamentos@vsgmarket.co.mz</strong>
      </div>

    </div><!-- /co-card pagamento -->
  </div><!-- /coluna esquerda -->
  ...
  <div>
    <div class="co-card" style="position:sticky;top:calc(var(--hdr-h)+16px)">
      ...
      <div class="order-items">
        ...
        <div class="oi-row">
          ...
          <div style="flex:1;min-width:0">
            <div class="oi-name" title="<?= esc($ci['product_name']) ?>"><?= esc($ci['product_name']) ?></div>
            <div class="oi-qty">Qtd: <?= (int)$ci['quantity'] ?></div>
          </div>
      ...
      <div class="sum-row">
        <span style="color:var(--ink-2)">Entrega</span>
        <span class="<?= $shipping_cost===0?'sum-free':'' ?>" style="font-weight:700">
          <?= $shipping_cost===0 ? 'Grátis' : $fmt($shipping_cost) ?>
        </span>
      </div>
      <div class="sum-row total">
        <span>Total</span>
        <span><?= $fmt($total_mzn) ?></span>
      </div>
...
<div class="pay-overlay" id="payOverlay">
  <div class="po-box">
    ...
    <div class="po-title" id="poTitle">A processar…</div>
    ...
    <button class="po-err-btn" id="poErrBtn" onclick="closeOverlay()">← Voltar e corrigir</button>
  </div>
</div>
...
<div id="successScreen">
  <div class="success-wrap">
    ...
    <div class="success-card" id="sCardNum">
      <div class="success-card-title">Número do Pedido</div>
      <div class="success-num">
        <span id="sOrderNum"></span>
        <button class="copy-btn" onclick="copyOrder()" title="Copiar"><i class="fa-regular fa-copy"></i></button>
      </div>
    </div>
    <div class="success-card" id="sCardPay" style="display:none">
      <div class="success-card-title">Pagamento</div>
      <div style="display:flex;flex-direction:column;gap:7px">
        <div style="display:flex;justify-content:space-between;font-size:13px">
          <span style="color:var(--ink-3)">Estado</span>
          <span style="color:#16a34a;font-weight:700"><i class="fa-solid fa-circle-check"></i> Aprovado</span>
        </div>
        <div style="display:flex;justify-content:space-between;font-size:13px">
          <span style="color:var(--ink-3)">Transacção</span>
          <span style="font-family:monospace;font-size:11px;font-weight:700" id="sTxnId"></span>
        </div>
        <div style="display:flex;justify-content:space-between;font-size:13px">
          <span style="color:var(--ink-3)">Método</span>
          <span style="font-weight:700" id="sPayMethod"></span>
        </div>
      </div>
    </div>
    <div class="success-card" id="sCardSteps">
      <div class="success-card-title">Próximos Passos</div>
      <div class="success-steps" id="sStepsList"></div>
    </div>
    <div class="success-btns" id="sBtns">
      <a href="pages/person/index.php" class="btn-orders"><i class="fa-solid fa-list-check"></i> Ver Pedidos</a>
      <a href="shopping.php" class="btn-shop2"><i class="fa-solid fa-border-all"></i> Continuar</a>
    </div>
  </div>
</div>
...
let   CSRF        = <?= json_encode($_SESSION['csrf_token']) ?>; // let — actualizado após cada resposta
const TOTAL_MZN   = <?= $total_mzn ?>;
const USER_PHONE  = <?= json_encode($prefill_phone_norm) ?>;

// URL base calculada no PHP — garante caminho correcto independentemente de onde
// o checkout.php esteja alojado (raiz, subpasta, etc.)
<?php
...
window.formatPhone = function(inp) {
  ...
  // Se começa com 258 removemos (já está no prefixo)
...
// ══════════════════════════════════════════════════════
// MOBILE MONEY — Status UI
// ══════════════════════════════════════════════════════
let pollInterval = null;
let pollSeconds  = 0;
let pollMax      = 120; // segundos máximos a esperar (2 min)
...
function showMmStatus(method, type, title, msg) {
  ...
  const icons = {waiting:'<i class="fa-solid fa-mobile-screen mm-status-icon"></i>',
                  polling:'<i class="fa-solid fa-spinner fa-spin mm-status-icon"></i>',
                  success:'<i class="fa-solid fa-circle-check mm-status-icon"></i>',
                  failed: '<i class="fa-solid fa-circle-xmark mm-status-icon"></i>'};
  el.innerHTML = (icons[type]||'') +
    '<div class="mm-status-text"><strong>'+escH(title)+'</strong>'+escH(msg)+'</div>';
}
function hideMmStatus(method) {
  const el=document.getElementById(method+'-status');
  if(el)el.className='mm-status';
}

// Timer visual (conic gradient countdown)
let timerEl = null;
function startTimer(secs) {
  ...
  const tick=setInterval(()=>{
    remaining--;
    const ring=timerEl.querySelector('.mm-timer-ring');
    const num =timerEl.querySelector('.mm-timer-num');
...
window.submitOrder = async function() {
  ...
  if (isMM) {
    ...
    await initiateMobileMoney(currentPay, fullPhone, {ship_name,ship_phone,ship_address,ship_city,ship_state,notes});
    return;
  }
...
// ══════════════════════════════════════════════════════
// MOBILE MONEY — Iniciar USSD Push
// ══════════════════════════════════════════════════════
async function initiateMobileMoney(method, phone, formData) {
  showMmStatus(method,'waiting',
    'A enviar pedido USSD…',
    'Receberá uma notificação no número '+phone+' para confirmar o pagamento.');
  const btn=document.getElementById('btnPlace');

  const fd = new FormData();
  fd.set('action','initiate_mm');
  fd.set('csrf_token', CSRF);
  fd.set('pay_method', method);
  fd.set('phone', phone);
  fd.set('amount', TOTAL_MZN);
  fd.set('order_ref', 'VSG-'+Date.now());
  fd.set('ship_name', formData.ship_name);
  fd.set('ship_phone', formData.ship_phone);
  fd.set('ship_address', formData.ship_address);
  fd.set('ship_city', formData.ship_city);
  fd.set('ship_state', formData.ship_state||'');
  fd.set('notes', formData.notes||'');
  fd.set('buy_now_id', BUY_NOW_ID);
  fd.set('buy_now_qty', BUY_NOW_QTY);
  ...
  try {
    ...
    showMmStatus(method,'failed','Erro de ligação',e.message+' · Verifique a sua internet e tente novamente.');
    btn.disabled=false; return;
  }

  if (!resp.ok) {
    showMmStatus(method,'failed','Erro',resp.msg||'Ocorreu um erro.');
    btn.disabled=false; return;
  }

  // Guardamos o conversation_id para polling
  currentConvId   = resp.conversation_id;
  currentPayToken = resp.pay_token;

  showMmStatus(method,'polling',
    '⏳ Aguardando confirmação PIN…',
    'Aprove o pagamento de MT '+TOTAL_MZN.toLocaleString('pt-MZ',{minimumFractionDigits:2})+' no seu telemóvel ('+phone+').');
  startTimer(pollMax);
  startPolling(method, currentConvId, currentPayToken, formData);
}

// ══════════════════════════════════════════════════════
// POLLING — verificar estado do pagamento
// ══════════════════════════════════════════════════════
function startPolling(method, convId, payToken, formData) {
  ...
  pollInterval = setInterval(async () => {
    ...
    if (pollSeconds >= pollMax) {
      ...
      showMmStatus(method,'failed','Tempo expirado','O utilizador não confirmou o pagamento a tempo. Tente novamente.');
      document.getElementById('btnPlace').disabled=false;
      return;
    }
    try {
      const r = await fetch(POLL_URL+'?conv_id='+encodeURIComponent(convId)+'&method='+method+'&token='+encodeURIComponent(payToken),{
      ...
      if (d.status==='confirmed') {
        ...
        // Criar pedido na DB agora que pagamento está confirmado
        showOverlay('A criar pedido…','O pagamento foi confirmado. A finalizar…');
        setStep(3);
        await submitToBackend({
          pay_method: method,
          phone: convId, // passamos o conv_id para o backend confirmar
          pay_token: payToken,
          mm_confirmed: '1',
        ...
        clearInterval(pollInterval); stopTimer();
        showMmStatus(method,'failed','Pagamento recusado',d.msg||'O utilizador cancelou ou o PIN foi incorreto.');
        document.getElementById('btnPlace').disabled=false;
      }
      // status==='pending' → continuar a fazer polling
    } catch(e) {
      // Rede instável — mostrar aviso mas continuar a tentar
      showMmStatus(method,'polling',
        '⏳ Aguardando confirmação PIN…',
        'Rede instável ('+e.message+'). A tentar novamente…');
    }
  }, 5000); // polling a cada 5s
}

// ══════════════════════════════════════════════════════
// SUBMIT PARA BACKEND (criar pedido DB)
// ══════════════════════════════════════════════════════
async function submitToBackend(extra) {
  const btn=document.getElementById('btnPlace');
  const fd = new FormData();
  fd.set('action','create_order');
  fd.set('csrf_token', CSRF);          // usa o token mais recente (let, não const)
  fd.set('buy_now_id', BUY_NOW_ID);
  fd.set('buy_now_qty', BUY_NOW_QTY);
  ...
  try {
    ...
    showOverlayError('Erro de ligação: '+e.message);
    btn.disabled=false; return;
  }

  // Actualizar CSRF para próxima operação (ex: polling → create_order)
  ...
  if(data.ok){
    ...
    showOverlayError(data.msg||'Ocorreu um erro. Tente novamente.');
    btn.disabled=false;
  }
}
...
function showSuccess(data) {
  ...
  document.getElementById('sSub').textContent = isCard
    ? 'O seu pagamento foi processado e o pedido confirmado.'
    : isMM
    ? 'Pagamento '+data.pay_method.toUpperCase()+' confirmado. O pedido está a ser processado.'
    : 'O pedido foi registado. Efectue a transferência para confirmar.';

  if((isCard||isMM)&&data.transaction_id){
    document.getElementById('sCardPay').style.display='block';
    document.getElementById('sTxnId').textContent    = data.transaction_id;
    document.getElementById('sPayMethod').textContent= data.pay_method.toUpperCase();
  }

  const stepsList = isManual ? [
    ['envelope','Receberá confirmação por email.'],
    ['building-columns','Efectue a transferência usando o número do pedido.'],
    ['check-circle','Após confirmarmos, o produto será enviado.'],
  ] : isMM ? [
    ['check-circle','Pagamento '+data.pay_method.toUpperCase()+' confirmado com sucesso.'],
    ['truck','O produto será enviado em 1-3 dias úteis.'],
    ['envelope','Receberá confirmação por email.'],
  ] : [
    ['check-circle','Pagamento aprovado pelo banco emissor.'],
    ['truck','O produto será enviado em 1-3 dias úteis.'],
    ['envelope','Receberá confirmação por email.'],
  ];
  document.getElementById('sStepsList').innerHTML=stepsList.map(([icon,text])=>
    `<div class="success-step"><i class="fa-solid fa-${icon}"></i><span class="success-step-text">${text}</span></div>`
  ).join('');
  ...
  const items=[
    ['sIcon',50],['sTitle',240],['sSub',370],['sCardNum',490],
    ['sCardPay',600],['sCardSteps',680],['sBtns',800],
  ];
...
window.copyOrder = function() {
  ...
  if(num)navigator.clipboard?.writeText(num).then(()=>{
    document.querySelectorAll('.copy-btn').forEach(b=>{b.innerHTML='<i class="fa-solid fa-check"></i>';setTimeout(()=>b.innerHTML='<i class="fa-regular fa-copy"></i>',2000);});
  });
};

// Limpar badge do carrinho ao sucesso
