...
<div class="whatsapp-container" id="whatsappContainer">
    ...
    <div class="conversations-panel">
        <div class="conversations-header">
            ...
            <div class="search-box">
                ...
                <input type="text" id="msgSearchInput" placeholder="Pesquisar conversas...">
            </div>
        </div>
        
        <div class="conversations-list" id="msgConversationsList">
            ...
            <?php else: ?>
                <?php foreach ($conversations as $conv): ?>
                    <div class="conversation-item" 
                         ...
                         data-sender-name="<?= htmlspecialchars($conv['sender_name']) ?>"
                         onclick="MessagingSystem.openChat(<?= $conv['sender_id'] ? $conv['sender_id'] : 'null' ?>, '<?= htmlspecialchars(addslashes($conv['sender_name'])) ?>')">
    ...
    <!-- Painel de Chat -->
    <div class="chat-panel" id="msgChatPanel">
...
window.MessagingSystem = (function() {
    ...
    async function openChat(senderId, senderName) {
        ...
        // Atualizar UI
        document.querySelectorAll('#msgConversationsList .conversation-item').forEach(item => {
            item.classList.remove('active');
        });
        const activeConv = document.querySelector(`[data-sender-id="${senderId === null ? 'system' : senderId}"]`);
        if (activeConv) activeConv.classList.add('active');
        
        const chatPanel = document.getElementById('msgChatPanel');
        chatPanel.innerHTML = '<div class="empty-chat"><i data-lucide="loader" class="spinning"></i><p>Carregando...</p></div>';
        if (typeof lucide !== 'undefined') lucide.createIcons();
        
        try {
            const response = await fetch(`actions/get_messages.php?sender_id=${senderId === null ? '' : senderId}`);
    ...
    function renderChat(senderId, senderName, messages, senderInfo) {
        ...
        const senderIdParam = senderId === null ? 'null' : senderId;
        ...
        chatPanel.innerHTML = `
            <div class="chat-header">
                ...
                <div class="chat-actions">
                    <button class="chat-action-btn" onclick="MessagingSystem.markAllAsRead(${senderIdParam})" title="Marcar todas como lidas">
                        <i data-lucide="check-check" style="width: 18px;"></i>
                    </button>
                    <button class="chat-action-btn" onclick="MessagingSystem.archiveConversation(${senderIdParam})" title="Arquivar">
                        <i data-lucide="archive" style="width: 18px;"></i>
                    </button>
                    <button class="chat-action-btn" onclick="MessagingSystem.deleteConversation(${senderIdParam})" title="Excluir" style="color: #ff3232;">
                        <i data-lucide="trash-2" style="width: 18px;"></i>
                    </button>
                </div>
            </div>
            
            <div class="messages-container" id="msgMessagesContainer">
    ...
    async function markConversationAsRead(senderId) {
        try {
            ...
            const badge = document.querySelector(`[data-sender-id="${senderId === null ? 'system' : senderId}"] .conv-unread-badge`);
    ...
    async function markAllAsRead(senderId) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                const convItem = document.querySelector(`[data-sender-id="${senderId === null ? 'system' : senderId}"]`);
                const senderName = convItem?.getAttribute('data-sender-name') || 'Usuário';
    ...
    async function deleteConversation(senderId) {
        if (!confirm('⚠️ Excluir TODAS as mensagens desta conversa? Não pode ser desfeito!')) return;
