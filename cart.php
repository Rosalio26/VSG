 ...
 * Modo normal:  página completa com header/footer
 * Modo embed:   só o conteúdo — quando chamado via AJAX (X-Requested-With)
 *               ou com ?embed=1 (pelo dashboard_person)
 *
 ...
 * ── DETECÇÃO DE EMBED ────────────────────────────────────────────────
 * Embed activo quando:
 *   1. Pedido AJAX (header X-Requested-With: XMLHttpRequest)
 *   2. Parâmetro ?embed=1 na URL
 *
 * No modo embed:
 *   • Sem <!DOCTYPE>, <html>, <head>, <body>
 *   • Sem top-strip, header, breadcrumb, footer
 *   • CSS injectado inline (só as regras do carrinho)
 *   • Links e redirects usam BASE_URL absoluto
 */
$is_embed = !empty($_SERVER['HTTP_X_REQUESTED_WITH'])
         || !empty($_GET['embed']);

/* ── BASE_URL para o embed ──────────────────────────────────────────
 * cart.php está na raiz → BASE_URL = /vsg (ou '' se na raiz do domínio)
 */
$_sn_parts  = explode('/', str_replace('\\', '/', $_SERVER['SCRIPT_NAME']));
array_pop($_sn_parts); // remove 'cart.php'
$base_url   = implode('/', $_sn_parts); // ex: '/vsg'
...
/*
 * Caminhos usados no HTML/JS — absolutos quando em embed
 * para funcionar correctamente dentro do dashboard.
 */
$path_shopping  = $is_embed ? $base_url . '/shopping.php'    : 'shopping.php';
$path_checkout  = $is_embed ? $base_url . '/checkout.php'    : 'checkout.php';
$path_login     = $is_embed ? $base_url . '/registration/login/login.php?redirect=' . urlencode($base_url . '/cart.php')
                             : 'registration/login/login.php?redirect=cart.php';
$path_register  = $is_embed ? $base_url . '/registration/register/register.php'
                             : 'registration/register/register.php';
$path_ajax_cart = $is_embed ? $base_url . '/ajax/ajax_cart.php'       : 'ajax/ajax_cart.php';
$path_ajax_guest= $is_embed ? $base_url . '/ajax/ajax_cart_guest.php' : 'ajax/ajax_cart_guest.php';
$path_product   = $is_embed ? $base_url . '/product.php'              : 'product.php';
?>
<?php if (!$is_embed): ?>
<!DOCTYPE html>
<html lang="pt-BR">
...
<style>
<?php if ($is_embed): ?>
/* ── Reset mínimo para embed (o dashboard já tem o reset global) ── */
#cart-embed-root *,#cart-embed-root *::before,#cart-embed-root *::after{box-sizing:border-box}
#cart-embed-root{font-family:'Plus Jakarta Sans',system-ui,sans-serif;font-size:14px;line-height:1.5;color:#1c2127}
<?php endif; ?>
:root {
...
/* ── Estilos do carrinho (partilhados por normal e embed) ── */
<?php if (!$is_embed): ?>
...
<?php endif; ?>
.guest-bar{<?= $is_embed ? '' : 'max-width:1360px;margin:0 auto;' ?>padding:0 <?= $is_embed ? '0' : '20px' ?> 14px}
.guest-bar-inner{display:flex;align-items:center;gap:12px;flex-wrap:wrap;background:linear-gradient(135deg,#eff6ff 0%,#f0fdf4 100%);border:1px solid #bfdbfe;border-radius:var(--r12);padding:12px 18px;font-size:13px;color:var(--ink-2)}
...
<?php if ($is_embed): ?>
/* ══ EMBED — layout compacto para o dashboard ══
   Tudo numa coluna, itens mais pequenos, botões inline
   sem ocupar a largura toda.
════════════════════════════════════════════════ */
.cart-wrap{display:flex;flex-direction:column;gap:14px;padding:0}

/* Título menor */
.section-title{font-size:14px;margin-bottom:10px}

/* Item do carrinho — mais compacto */
.cart-item{padding:10px 12px;gap:10px;border-radius:var(--r10)}
.ci-img{width:56px;height:56px;border-radius:var(--r8)}
.ci-cat{font-size:9.5px;margin-bottom:2px}
.ci-name{font-size:12px;margin-bottom:2px}
.ci-sup{font-size:11px;margin-bottom:4px}
.ci-price-row{margin-top:6px;gap:6px}
.ci-unit-price{font-size:11px}
.ci-total{font-size:13px}
.ci-remove{width:24px;height:24px;font-size:11px}

/* Controlo de quantidade — mais pequeno */
.qty-ctrl{height:26px}
.qty-btn{width:24px;font-size:11px}
.qty-val{width:32px;font-size:12px}

/* Acções — inline em vez de stack */
#cartActions{margin-top:8px}
#cartActions a,#cartActions button{font-size:11px;padding:5px 10px}

/* Summary — compacto */
.cart-summary{padding:14px;border-radius:var(--r10)}
.summary-title{font-size:13px;margin-bottom:10px}
.sum-row{font-size:12.5px;padding:3px 0}
.sum-row.total{margin-top:6px;padding-top:10px}
.sum-row.total .sum-lbl{font-size:13px}
.sum-row.total .sum-val{font-size:1.1rem}

/* Botões — inline e proporcionais */
.cart-btns-row{display:flex;gap:8px;margin-top:12px;align-items:center}
.btn-checkout{
  width:auto;flex:1;
  padding:9px 16px;
...
.trust-pills{margin-top:10px;padding-top:10px;gap:5px}
...
.cart-item{background:var(--sur);border:1px solid var(--bdr);border-radius:var(--r12);padding:16px;display:flex;gap:16px;align-items:flex-start;transition:border-color .15s,box-shadow .15s,opacity .2s,transform .2s;position:relative;overflow:hidden}
.cart-item:hover{border-color:rgba(0,185,107,.3);box-shadow:var(--sh-sm)}
.cart-item.unavailable{opacity:.6}
.cart-item.unavailable::after{content:'Produto indisponível';position:absolute;top:10px;right:10px;background:var(--red);color:#fff;font-size:10px;font-weight:700;padding:3px 8px;border-radius:var(--r99)}
...
.qty-btn{width:32px;height:100%;display:grid;place-items:center;font-size:13px;color:var(--ink-2);transition:background .15s,color .15s;border:none;background:none;cursor:pointer}
...
.ci-remove{width:30px;height:30px;border-radius:50%;display:grid;place-items:center;color:var(--ink-3);font-size:13px;transition:background .15s,color .15s;border:none;background:none;cursor:pointer}
...
.cart-summary{background:var(--sur);border:1px solid var(--bdr);border-radius:var(--r16);padding:20px;<?= $is_embed ? '' : 'position:sticky;top:calc(var(--hdr-h)+16px)' ?>}
...
.btn-checkout{width:100%;padding:14px;margin-top:18px;background:var(--gr);border-radius:var(--r10);color:#fff;font-size:15px;font-weight:800;display:flex;align-items:center;justify-content:center;gap:8px;transition:background .15s,transform .1s;border:none;cursor:pointer;font-family:var(--font)}
...
.btn-continue{width:100%;padding:10px;margin-top:8px;border:1.5px solid var(--bdr);border-radius:var(--r10);color:var(--ink-2);font-size:13px;font-weight:600;display:flex;align-items:center;justify-content:center;gap:7px;transition:border-color .15s,color .15s,background .15s;background:none;font-family:var(--font);cursor:pointer}
...
@media(max-width:600px){.cart-wrap{padding:0 12px 80px}.ci-img{width:68px;height:68px}}
</style>

<?php if (!$is_embed): /* ══ HEADER NORMAL ══ */ ?>
<div class="top-strip">
  <div class="ts-in">
...
<?php endif; /* fim header normal */ ?>

<?php /* Wrapper para modo embed — isola o CSS */ ?>
<?php if ($is_embed): ?><div id="cart-embed-root"><?php endif; ?>
...
<div class="guest-bar" id="guestBar" style="display:none">
  <div class="guest-bar-inner">
    ...
    <div class="guest-bar-text">
      ...
      Inicie sessão para guardar permanentemente e finalizar a compra.
    </div>
    <div class="guest-bar-actions">
      <a href="<?= esc($path_login) ?>" class="gbtn gbtn-login">
        <i class="fa-solid fa-right-to-bracket"></i> Entrar
      </a>
      <a href="<?= esc($path_register) ?>" class="gbtn gbtn-reg">Criar conta</a>
    </div>
  </div>
</div>
<?php endif; ?>
...
<div class="cart-wrap">
  <div>
    ...
    <div id="cartActions" style="display:none;justify-content:space-between;align-items:center;margin-top:16px;flex-wrap:wrap;gap:10px">
      <a href="<?= esc($path_shopping) ?>" style="display:inline-flex;align-items:center;gap:6px;font-size:13px;font-weight:600;color:var(--ink-3);transition:color .15s">
        <i class="fa-solid fa-arrow-left"></i> Continuar a comprar
      </a>
      <button onclick="Cart.clear()" style="display:inline-flex;align-items:center;gap:6px;font-size:13px;font-weight:600;color:var(--red);padding:7px 14px;border:1.5px solid #fecaca;border-radius:var(--r8);background:none;cursor:pointer;font-family:inherit">
  ...
  <div>
    <div class="cart-summary">
      ...
      <div class="cart-btns-row">
        ...
        <button onclick="location.href='<?= esc($path_shopping) ?>'" class="btn-continue">
          <i class="fa-solid fa-arrow-left"></i> Continuar
        </button>
      </div>
...
<?php if ($is_embed): ?></div><!-- /cart-embed-root --><?php endif; ?>
...
function updateGlobalBadge(n){
  document.querySelectorAll('.cart-badge,[data-cart-count],.cart-count-badge').forEach(b=>{
...
const LS = {
  ...
  set(d){ try{ localStorage.setItem(LS_KEY,JSON.stringify(d)); }catch(e){} },
  count(){ const d=this.get(); return Object.values(d).reduce((a,i)=>a+(i.qty||0),0); },
  add(pid,qty,meta){ const d=this.get(); const cur=d[pid]||{qty:0}; d[pid]={...meta,qty:Math.min((cur.qty||0)+qty,meta.stock||99)}; this.set(d); return d; },
  update(pid,qty){ const d=this.get(); if(!d[pid]) return d; if(qty<=0){delete d[pid];}else{d[pid].qty=Math.min(qty,d[pid].stock||99);} this.set(d); return d; },
  remove(pid){ const d=this.get(); delete d[pid]; this.set(d); return d; },
  clear(){ this.set({}); }
};

function ajax(body){
  return fetch(PATH_AJAX_CART,{
    method:'POST',
...
function buildItemHTML(item){
  ...
  <div class="cart-item${available?'':' unavailable'}" id="item-${esc(itemId)}" data-pid="${pid}" data-price="${price}" data-stock="${stock}">
    <div class="ci-img" onclick="location.href='${PATH_PRODUCT}?id=${pid}'">
    ...
    <div class="ci-info">
      ...
      <div class="ci-name" onclick="location.href='${PATH_PRODUCT}?id=${pid}'">${name}</div>
...
function renderSummary(items){
  let sub=0;
  items.forEach(i=>{ sub+=parseFloat(i.item_price||i.price||0)*parseInt(i.quantity||i.qty||1); });
  const ship = sub>=2500?0:(sub>0?150:0);
  const total= sub+ship;
  document.getElementById('sumSubtotal').textContent = items.length?fmt(sub):'—';
  ...
  if(items.length){
    shipEl.textContent=ship===0?'Grátis':fmt(ship);
    shipEl.className='sum-val'+(ship===0?' sum-free':'');
    if(ship>0&&noteTxt){noteTxt.textContent='Falta '+fmt(2500-sub)+' para entrega grátis';noteEl.style.display='flex';}
    else if(noteEl){noteEl.style.display='none';}
  }else{shipEl.textContent='—';if(noteEl)noteEl.style.display='none';}
  ...
  if(items.length===0){
    container.innerHTML=`
      <div class="cart-empty">
        ...
        <a href="${PATH_SHOPPING}" class="btn-shop"><i class="fa-solid fa-border-all"></i> Explorar Produtos</a>
      </div>`;
    if(actions) actions.style.display='none';
    if(guestBar) guestBar.style.display='none';
  }else{
    const list=document.createElement('div');
    list.className='cart-list'; list.id='cartList';
...
window.Cart = {
  load(){
    if(IS_LOGGED){
      ...
      fetch(PATH_AJAX_GUEST+'?action=resolve&pids='+pids.join(','),{headers:{'X-Requested-With':'XMLHttpRequest'}})
        .then(r=>r.json())
        .then(d=>{
          if(!d.success){renderCart([]);return;}
          const items=d.products.map(p=>({...p,quantity:stored[p.product_id]?.qty||1,item_price:p.preco}));
          renderCart(items);
        })
        .catch(()=>renderCart([]));
    }
  ...
  updateItem(itemId,qty,pid){
    ...
    const qtyInp=el?.querySelector('.qty-val');
  ...
  removeItem(itemId,pid){
    ...
    const done=()=>{
      el.remove(); this._recalcSummary();
      if(!document.querySelectorAll('.cart-item').length) renderCart([]);
  ...
  _recalcSummary(){
    ...
    document.querySelectorAll('.cart-item').forEach(el=>{
      items.push({item_price:parseFloat(el.dataset.price)||0,quantity:parseInt(el.querySelector('.qty-val')?.value||0)});
    });
    renderSummary(items);
  },
  _mergeGuestOnLogin(){
...
window.CartGlobal = {
  ...
  _BASE:<?= json_encode($base_url) ?>,
  add(pid,qty,meta){
    if(this._IS_LOGGED){
      return fetch(this._BASE+'/ajax/ajax_cart.php',{method:'POST',
        headers:{'Content-Type':'application/x-www-form-urlencoded','X-Requested-With':'XMLHttpRequest'},
      ...
      try{
        ...
        document.querySelectorAll('.cart-badge,[data-cart-count],.cart-count-badge').forEach(b=>{b.textContent=total>99?'99+':total;b.style.display=total>0?'':'none';});
      }catch(e){}
